...
compile-tests:
	mkdir -p $(BIN)
	$(JAVAC) -d $(BIN) -cp "$(LIB)/junit-4.13.2.jar:$(LIB)/hamcrest-core-1.3.jar" $(SRC)/main/*.java $(SRC)/$(TEST)/*.java
...
_run-content:
	$(JAVA) -cp $(BIN) ContentServer localhost:4567 src/main/content/IDS60901.txt
